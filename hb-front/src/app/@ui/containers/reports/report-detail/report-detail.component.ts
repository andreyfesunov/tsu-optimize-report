      ...
      <app-content *ngIf="id$ | async as id" class="report-detail">
        <div class="report-detail-actions">
          ...
          <div class="report-hours-counter">
            Осталось заполнить часов: План - {{ (plan$ | async) ?? "..." }}, Факт - {{ (fact$ | async) ?? "..." }}
          </div>
        </div>
...
export class ReportDetailComponent extends SubscriptionController {
  ...
  protected readonly state$ = this.id$.pipe(map((id) => this._reportStateFactory.create(
    id,
    this.spinner,
    this._destroyRef
  )), shareReplay({
  ...
  protected readonly plan$ = this.state$.pipe(
    switchMap((state) => combineLatest([state.report$, state.states$.pipe(
      map((states) => states.map(x => x.states$)),
      switchMap((states) => combineLatest(states)),
      map((states) => states.reduce((acc, cur) => [...acc, ...cur], [])),
      switchMap((states) => combineLatest([
        ...states.map(x => x.lessonFormStates$.pipe(switchMap(x => {
          if (x.length === 0) {
            return of([]);
          }

          return combineLatest(x.map(value => value.form.controls.plan.valueChanges.pipe(startWith(value.form.controls.plan.value))));
        }))),
        ...states.map(x => x.commentFormStates$.pipe(switchMap(x => {
          if (x.length === 0) {
            return of([]);
          }
    ...
    switchMap((state) => combineLatest([state.report$, state.states$.pipe(
      ...
      switchMap((states) => combineLatest([
        ...states.map(x => x.lessonFormStates$.pipe(switchMap(x => {
          ...
          return combineLatest(x.map(value => value.form.controls.fact.valueChanges.pipe(startWith(value.form.controls.fact.value))));
        }))),
        ...states.map(x => x.commentFormStates$.pipe(switchMap(x => {
          if (x.length === 0) {
            return of([]);
