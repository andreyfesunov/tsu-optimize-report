    ...
    template: `
        ...
        >
            <ng-container *ngIf="item" [ngSwitch]="col.id">
                ...
                <ng-container
                        *ngSwitchCase="ReportsTableRowItemField.INSTITUTE">{{ item.state.department.institute.name }}</ng-container>
                <ng-container
                        *ngSwitchCase="ReportsTableRowItemField.DEPARTMENT">{{ item.state.department.name }}</ng-container>
                <ng-container *ngSwitchCase="ReportsTableRowItemField.HOURS">{{ item.state.hours }}</ng-container>
                <ng-container
                        *ngSwitchCase="ReportsTableRowItemField.START_DATE">{{ item.state.startDate }}</ng-container>
