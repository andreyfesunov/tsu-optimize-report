...
export class ReportsService {
  ...
  public search(
    dto: IPaginationRequest,
  ): Observable<IPagination<IReportListItem>> {
    return concat(of(0), this._reload$).pipe(
      switchMap(() =>
        this._httpClient.post<IPagination<IReportListItem>>(
          `${this._apiRoot}/Report/search`,
          dto,
        ),
      ),
    );
  ...
  public create(id: string, data: FormData): Observable<boolean> {
    return this._httpClient
      .post<boolean>(`${this._apiRoot}/Report/${id}/CreateReport`, data)
  ...
  ): Observable<IReportDetail> {
    return this._httpClient.get<IReportDetail>(
      `${this._apiRoot}/Report/${id}/${semesterId.toString()}`,
    );
  }

  public export(id: string): Observable<Blob> {
    return this._httpClient.get<Blob>(
      `${this._apiRoot}/Report/${id}/ExportReport`,
      {
  ...
  public eventTypes(reportId: string): Observable<readonly IEventType[]> {
    return this._httpClient.get<readonly IEventType[]>(
      `${this._apiRoot}/Report/${reportId}/event-type`,
    );
  }
}
