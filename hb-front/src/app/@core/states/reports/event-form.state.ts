...
export class EventFormState {
  public readonly lessonFormStates$: BehaviorSubject<LessonFormState[]> =
    new BehaviorSubject<LessonFormState[]>([]);
  /** Comment form state */

  public readonly commentFormStates$: BehaviorSubject<CommentFormState[]> =
    new BehaviorSubject<CommentFormState[]>([]);
  /** Event data */

  public readonly deleteDisabled$ = combineLatest([
    this.commentFormStates$,
    this.lessonFormStates$,
  ]).pipe(map(([comments, lessons]) => comments.length + lessons.length > 0));
  public readonly events$: BehaviorSubject<IEventType[]> = new BehaviorSubject<
    IEventType[]
  >([]);
  ...
  private readonly _typesMemo$: { [key: number]: Observable<ILessonType[]> } =
    {};
  private readonly _lessonControlsChanges$: Observable<(string | null)[]> =
    this.lessonFormStates$.pipe(
      distinctUntilChanged((prev, curr) => prev.length === curr.length),
      map(
        (states) =>
          new FormArray(states.map((state) => state.form.controls.lessonType)),
      ),
  ...
  public readonly canAddLesson$: Observable<boolean> = combineLatest([
    this.lessonFormStates$,
    this._types$,
  ]).pipe(map(([states, types]) => states.length < types.length));
  private readonly _id$: BehaviorSubject<string | null> = new BehaviorSubject<
    string | null
  >(null);

  public readonly editable$: Observable<boolean> = this._id$.pipe(
    map((v) => !!v),
  );
  ...
  public addLesson(lesson: ILesson | null = null): void {
    ...
    const state = this._eventStateFactory.createLesson(
      eventId,
      lesson,
      this._destroyRef,
    );

    this._availableTypes$(index)
      .pipe(takeUntilDestroyed(this._destroyRef))
      .subscribe(state.types$);

    this.lessonFormStates$.next([...this.lessonFormStates$.value, state]);
  }

  public addComment(comment: IComment | null = null): void {
    const eventId = required(this._id$.value);
    const state = this._eventStateFactory.createComment(
      eventId,
      comment,
      this._destroyRef,
    );

    this.commentFormStates$.next([...this.commentFormStates$.value, state]);
  }
  ...
  public deleteComment(index: number): void {
    ...
    if (deleted) {
      this._eventsService
        .deleteComment(deleted.id)
        .pipe(takeUntilDestroyed(this._destroyRef))
        .subscribe(() =>
          this.commentFormStates$.next(
            this.commentFormStates$.value.filter(
  ...
  public deleteLesson(index: number): void {
    ...
    if (deleted) {
      this._eventsService
        .deleteLesson(deleted.id)
        .pipe(takeUntilDestroyed(this._destroyRef))
        .subscribe(() =>
          this.lessonFormStates$.next(
            this.lessonFormStates$.value.filter(
              (_, arrIndex) => arrIndex !== index,
            ),
          ),
        );
  ...
  private readonly _availableTypes$ = (index: number) =>
    index in this._typesMemo$
      ? this._typesMemo$[index]
      : (this._typesMemo$[index] = this._lessonControlsChanges$.pipe(
          switchMap((typeChanges) =>
            this._types$.pipe(
              map((types) => {
                const selectedTypes = typeChanges.filter(
                  (v): v is string => !!v,
                );
